...
function read_lxc_template(){
    ...
    if [[ -n $last_lxc_version ]]; then
        ...
        os_list=$(echo "$image_list" | sed "s/https\:\/\/github.com\/hana395\/OsMutation\/releases\/download\/${last_lxc_version}\///g" | sed "s/\.tar\.gz//g")
        echo "$os_list" | nl
...
function main(){
    ...
    if [ "$cttype" == 'openvz' ] ; then
        ...
        curl -qo OsMutationKvm.sh https://raw.githubusercontent.com/hana395/OsMutation/main/OsMutationKvm.sh
